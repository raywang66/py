...
from PySide6.QtGui import QPixmap, QImage, QIcon, QPalette, QColor
...
Distribution:
  ...
  Highlights (>70%): {np.sum(point_cloud[:, 2] > 0.7) / len(point_cloud) * 100:.1f}%
    ...
    def _show_visualization(self):
        ...
        viz_window = QWidget()
        viz_window.setWindowTitle(f"Visualization - {self.current_photo.name}")
        viz_window.setGeometry(150, 150, 1200, 700)
        
        layout = QVBoxLayout(viz_window)
        
        # Title
        title = QLabel("🎨 Skin Tone Visualization")
        ...
        left_panel = QGroupBox("Detected Face Mask")
        left_layout = QVBoxLayout(left_panel)
        
        face_label = QLabel()
        face_label.setAlignment(Qt.AlignCenter)
        face_label.setStyleSheet("background-color: #2a2a2a; border: 1px solid #444;")
        
        # Create face mask visualization
        import cv2
        overlay = self.current_photo_rgb.copy()
        
        # Create colored mask overlay (semi-transparent teal)
        mask_colored = np.zeros_like(overlay)
        mask_colored[self.current_mask == 1] = [78, 205, 196]  # Teal color
        ...
        # Mask stats
        mask_info = QLabel(f"Mask covers {self.current_mask.sum() / self.current_mask.size * 100:.2f}% of image\n"
                          f"Excludes: eyes, eyebrows, lips, facial hair")
        ...
        right_panel = QGroupBox("3D HSL Cylindrical Wedge (H: 15-25°)")
        right_layout = QVBoxLayout(right_panel)
        
        if self.renderer_3d:
            # Render 3D visualization
            try:
                self.renderer_3d.set_point_cloud(self.point_cloud, color_mode='hsl')
                self.renderer_3d.render()
                render_img = self.renderer_3d.get_image()
                
                render_label = QLabel()
                render_label.setAlignment(Qt.AlignCenter)
                render_label.setStyleSheet("background-color: #2a2a2a; border: 1px solid #444;")
                ...
                render_info = QLabel(f"{len(self.point_cloud):,} points visualized\n"
                                    f"Color: HSL mapping to RGB")
                render_info.setStyleSheet("color: #999; font-size: 11px; padding: 5px;")
                render_info.setAlignment(Qt.AlignCenter)
                right_layout.addWidget(render_info)
                
            except Exception as e:
                logger.error(f"3D rendering error: {e}", exc_info=True)
                error_label = QLabel(f"3D rendering error:\n{str(e)}")
                error_label.setStyleSheet("color: #ff6b6b; padding: 20px;")
                error_label.setAlignment(Qt.AlignCenter)
                right_layout.addWidget(error_label)
        else:
            # Taichi not available
            info_label = QLabel("3D visualization requires Taichi\n\nInstall with: pip install taichi")
...
def main():
    ...
    app.setApplicationVersion(CC_VERSION)
    ...
    # Show RAW support status
    if not RAWPY_AVAILABLE:
        logger.warning("rawpy not installed - RAW file support disabled")
